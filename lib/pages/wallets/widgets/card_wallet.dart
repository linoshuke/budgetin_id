...
class _WalletCardState extends State<WalletCard> {
  ...
  void initState() {
    ...
    _statsStream = firestoreService.getWalletStatsStream(widget.wallet.id, widget.wallet.displayPreference);
  }
  ...
  void _showEditWalletDialog(BuildContext context) {
    final TextEditingController nameController = TextEditingController(text: widget.wallet.walletName);
          ...
          ElevatedButton(
            onPressed: () {
              if (formKey.currentState!.validate()) {
                context.read<FirestoreService>().updateWalletName(widget.wallet.id, nameController.text.trim());
      ...
      return StatefulBuilder(builder: (context, setDialogState) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text("Tindakan ini tidak dapat diurungkan dan akan menghapus semua transaksi di dalamnya.\n\nUntuk konfirmasi, ketik \"${widget.wallet.walletName}\" di bawah ini."),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: !isButtonEnabled ? null : () {
                if (formKey.currentState!.validate()) {
                  context.read<FirestoreService>().deleteWallet(widget.wallet.id);
                  Navigator.of(context).pop();
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Dompet "${widget.wallet.walletName}" telah dihapus.'))
                  );
                }
              },
                  ...
                  PopupMenuButton<String>(
                    ...
                    itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                      const PopupMenuItem<String>(
                        value: 'edit',
                        child: ListTile(leading: Icon(Icons.edit), title: Text('Ubah Nama')),
                      ),
                      const PopupMenuItem<String>(
                        value: 'delete',
                        child: ListTile(leading: Icon(Icons.delete_outline, color: Colors.red), title: Text('Hapus Dompet', style: TextStyle(color: Colors.red))),
                      ),
                    ],
              ...
              StreamBuilder<Map<String, double>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError) {
                     return const Center(child: Icon(Icons.error_outline, color: Colors.white70, size: 20));
                  }
